...
task Release -depends Test {
    ...
    cp $build_output\TweetsDotNet.dll $release_output
}
...
task Nuget -depends Test {
	...
	mkdir $lib | out-null
    cp $build_output\TweetsDotNet.dll $lib
	$version = [System.Diagnostics.FileVersionInfo]::GetVersionInfo("$build_output\TweetsDotNet.dll").FileVersion;
    Write-Host "Version: $version"
	Generate-NuSpecFile -file "$nuget_output\TweetsDotNet.nuspec" -version "$version"
	$old = pwd
	cd $nuget_output
	Exec { &"$nuget_tool" pack TweetsDotNet.nuspec }
	cd $old	
}
...
task Compile -depends Clean { 
    ...
    Exec { msbuild "$code_dir\TweetsDotNet.sln" /t:Build /p:Configuration=Release /v:quiet /p:OutDir=$build_output } 
}
...
task Clean { 
    ...
    Exec { msbuild "$code_dir\TweetsDotNet.sln" /t:Clean /p:Configuration=Release /v:quiet }
}